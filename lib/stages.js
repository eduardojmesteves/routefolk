...
export function validateCustomMapsUrl(value) {
  ...
  if (!ALLOWED_MAPS_HOSTS.has(parsed.hostname)) {
    throw new Error('Only Google Maps URLs are allowed (google.com or maps.app.goo.gl).');
  }
...
/** Swap two stages' order_index values. */
export async function swapStageOrder(stageA, stageB) {
  const { error: e1 } = await supabase
    .from('stages')
    .update({ order_index: stageB.order_index })
    .eq('id', stageA.id);
  if (e1) throw e1;
  const { error: e2 } = await supabase
    .from('stages')
    .update({ order_index: stageA.order_index })
    .eq('id', stageB.id);
  if (e2) throw e2;
}
