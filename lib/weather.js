...
const CACHE_KEY_PREFIX = 'rf:wx:v3:';
...
export async function fetchDailyForecast(lat, lng, date) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('[routefolk weather] Open-Meteo failed', { status: res.status, url: url.toString(), body });
      writeCache(key, null);
      return null;
    }
    ...
    const index = d.time.indexOf(safeDate);
    if (index === -1) {
      writeCache(key, null);
      return null;
    }

    const code = valueAt(d.weather_code, index);
    ...
    const value = {
      ...
      label: meta.label,
      icon: meta.icon,
      tempMin: valueAt(d.temperature_2m_min, index),
      tempMax: valueAt(d.temperature_2m_max, index),
      precipMm: valueAt(d.precipitation_sum, index),
      precipProb: valueAt(d.precipitation_probability_max, index),
      windKmh: valueAt(d.wind_speed_10m_max, index),
    };
