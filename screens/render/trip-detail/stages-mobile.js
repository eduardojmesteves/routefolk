...
/** Per-entry [✎] [✕] icon buttons rendered as the third grid cell of
 *  .rf-clean-note. Hidden on archived trips, disabled when offline. */
function mobileEntryActionsHtml(trip, entry, stage) {
  if (isArchivedTrip(trip)) return '';
  const dis = writeDisabledAttr();
  return `<div class="rf-clean-note-actions"><button data-action="rf-v2-edit-entry" data-entry-id="${esc(entry.id)}" data-stage-id="${esc(stage.id)}" type="button" title="Edit entry"${dis}>✎</button><button class="danger" data-action="rf-v2-delete-entry" data-entry-id="${esc(entry.id)}" data-stage-id="${esc(stage.id)}" type="button" title="Delete entry"${dis}>✕</button></div>`;
}

/** Drill-in [Edit stage] [Delete] pills, rendered before Navigate.
 *  Hidden on archived trips. Per Scene D of the v3 mockup, the
 *  drill-in does NOT carry reorder controls — those live on the
 *  stages-list card footer only. */
function mobileStageActionPillsHtml(trip, stage) {
  if (isArchivedTrip(trip)) return '';
  const dis = writeDisabledAttr();
  return `<button data-action="rf-v2-edit-stage" data-stage-id="${esc(stage.id)}" type="button"${dis}>Edit stage</button><button class="is-danger" data-action="rf-v2-delete-stage" data-stage-id="${esc(stage.id)}" type="button"${dis}>Delete</button>`;
}
...
export function renderMobileJournal(trip, { screen }) {
  ...
  const archived = isArchivedTrip(trip);
  const dis = writeDisabledAttr();
  const addJournal = archived ? '' : `<button data-action="rf-m2-add-journal"${dis}>+ Add</button>`;
  const addExpense = archived ? '' : `<button data-action="rf-v2-add-stage-expense" data-stage-id="${esc(stage.id)}"${dis}>+ Add</button>`;
  return screen(`<main class="rf-clean-page"><button class="rf-clean-back" data-action="rf-m2-back-to-stages">← ${esc(trip.title || 'Trip')}</button><div class="rf-clean-kicker">Stage · ${esc(fmtDate(stage.planned_date) || '')}</div><h1 class="rf-clean-title">${esc(stage.start_location || 'Start')} <em>to</em> ${esc(stage.end_location || 'End')}</h1><div class="rf-m2-stage-actions">${mobileStageActionPillsHtml(trip, stage)}${mobileNavigateHtml(trip, stage)}</div><p>${esc(stage.notes || '')}</p>${mobileWeatherHtml(stage)}<div class="rf-clean-section-head"><h2>The day's notes</h2>${addJournal}</div>${entries.map((entry, i) => `<article class="rf-clean-note"><span>${i + 1}</span><div><small>A ${esc(entry.entry_type || 'note')}</small><strong>${esc(entry.title || 'Untitled')}</strong>${entry.location ? `<em>at ${esc(entry.location)}</em>` : ''}</div>${mobileEntryActionsHtml(trip, entry, stage)}</article>`).join('') || '<div class="rf-clean-empty">No entries yet.</div>'}<div class="rf-clean-section-head"><h2>Stage costs</h2>${addExpense}</div>${stageExpenses.map((expense) => `<article class="rf-clean-expense"><div><strong>${esc(categoryLabel(expense.category))}</strong><small>${esc(payerName(expense.user_id))}</small></div><b>${fmtEuro(expense.amount || 0)}</b></article>`).join('') || '<div class="rf-clean-empty">No costs assigned to this stage.</div>'}<section class="rf-v2-gpx-section">${gpxPanelHtml(trip, stage, tracks)}</section></main>${STATE.navSheet && sheetStage ? navigateSheetHtml(sheetStage, STATE.navSheet) : ''}`);
}
