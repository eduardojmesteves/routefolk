...
The Cloudflare dashboard alternative is **Workers & Pages → Create → Pages →
Direct Upload**. A browser on another computer cannot drag files directly from
the home server, so Wrangler is normally the safer path for this rehearsal.
Never select the existing production Pages project.
