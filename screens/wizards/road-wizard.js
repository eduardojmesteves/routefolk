...
function stageLinkCardHtml(stageId, tripTitle, stageNo, date) {
  return `<div class="rf-road-link-card" data-stage-id="${esc(stageId)}"><span>${esc(tripTitle)} · Stage ${esc(stageNo || '')}</span><b class="rf-mono">${esc(date || '')}</b><button type="button" class="rf-road-link-remove" data-action="rf-road-link-remove" data-stage-id="${esc(stageId)}" aria-label="Remove link">×</button><input type="hidden" name="road-stage-link" value="${esc(stageId)}"></div>`;
}

function stagePickerRowHtml(stage, tripTitle) {
  return `<button type="button" class="rf-stage-link-row" data-action="rf-road-link-add" data-stage-id="${esc(stage.id)}"><strong>${esc(stage.start_location || 'Start')} <span class="rf-desktop-stage-to">to</span> ${esc(stage.end_location || 'End')}</strong><small>${esc(tripTitle)} · ${esc(fmtDate(stage.planned_date) || 'No date yet')}</small></button>`;
}
...
function stagePickerHtml(excludeIds) {
  ...
  const groups = trips.map((trip) => {
    ...
    return `<div class="rf-stage-link-group"><div class="rf-stage-link-trip">${esc(trip.title)}</div>${rows}</div>`;
  }).filter(Boolean).join('');
  if (groups) return groups;
  const stillLoading = trips.length && trips.some((trip) => !STATE.stagesByTrip[trip.id]);
  return `<p class="rf-desktop-aside-sub">${stillLoading ? 'Loading your stages…' : 'No more stages to link.'}</p>`;
}
...
function roadLinkedSectionHtml(linkedIds, pickerOpen = false) {
  ...
  return `<div class="rw-linked-stages" id="road-linked-stages">${cards}</div><button type="button" class="rf-desktop-btn is-dashed" data-action="rf-road-link-stage-toggle" aria-expanded="${pickerOpen ? 'true' : 'false'}" aria-controls="road-stage-picker">+ Link another stage</button><div class="rf-stage-linker" id="road-stage-picker" ${pickerOpen ? '' : 'hidden'}>${stagePickerHtml(linkedIds)}</div>`;
}

function stageLinkerHtml(linkedStageIds) {
  return `<div id="road-linker-body">${roadLinkedSectionHtml(linkedStageIds)}</div>`;
}

function currentLinkedStageIds() {
  return new Set([...(wizardHost()?.querySelectorAll('input[name="road-stage-link"]') || [])].map((el) => el.value));
}

function refreshRoadStageLinker(linkedIds, pickerOpen) {
  const body = wizardHost()?.querySelector('#road-linker-body');
  if (!body) return;
...
export function toggleRoadStagePicker() {
  const picker = wizardHost()?.querySelector('#road-stage-picker');
...
export function roadPreviewHtml(suffix = '') {
  const name = fieldValue(`road-name${suffix}`) || 'Unnamed road';
  const from = fieldValue(`road-from${suffix}`);
  const to = fieldValue(`road-to${suffix}`);
  const notes = fieldValue(`road-notes${suffix}`);
  const rating = Number(field(`road-rating${suffix}`)?.value) || 0;
  const stars = [1, 2, 3, 4, 5].map((n) => `<span class="rf-star-mini ${n <= rating ? 'is-filled' : ''}">${starSvg()}</span>`).join('');
  return `<article class="rf-road-card rf-preview-card"><div class="rf-road-card-head"><strong>${esc(name)}</strong><span class="rf-road-stars">${stars}</span></div>${from || to ? `<div class="rf-road-connects">${esc(from || 'Start')} <span class="rf-route-arrow">→</span> ${esc(to || 'End')}</div>` : ''}${notes ? `<p class="rf-road-notes">${esc(notes)}</p>` : ''}</article>`;
}

// ---- Wizard markup ------------------------------------------------------
export function roadCreateWizardHtml() {
  const sections = [
    narrativeSection('road-section-name', 'Which road is it?', 'The route number or name.', row('road-name', 'Road number or name', input('road-name', '', 'placeholder="N-260"'))),
    narrativeSection('road-section-connect', 'What does it connect?', 'Region to region.', connectedRouteRow('road-from', 'road-to', '', '', 'placeholder="Sort"', 'placeholder="Núria"')),
    narrativeSection('road-section-notes', 'Anything worth noting?', '', row('road-notes', 'Notes', textarea('road-notes', '', 'placeholder="Hairpins, gravel patch after the tunnel..."'))),
    narrativeSection('road-section-rating', 'How many stars?', 'Your own rating — pins it higher in your list.', starPickerHtml('road-rating', 0)),
    narrativeSection('road-section-stages', 'Rode it on a stage?', "Optional — link one or more stages you've ridden this road on. Dates fill in automatically.", stageLinkerHtml(new Set())),
  ];
  return narrativeShellHtml({
    id: 'rf-road-create-title',
    kicker: 'New road',
    ...
    errorId: 'road-create-error',
    saveAction: 'rf-save-road',
    saveLabel: 'Save road',
  });
}
...
export function roadEditWizardHtml() {
  ...
  const sections = [
    narrativeSection('road-edit-section-name', 'Which road is it?', 'The route number or name.', row('road-name-edit', 'Road number or name', input('road-name-edit', road.road_number_or_name || '', 'placeholder="N-260"'))),
    narrativeSection('road-edit-section-connect', 'What does it connect?', 'Region to region.', connectedRouteRow('road-from-edit', 'road-to-edit', road.connection_from || '', road.connection_to || '')),
    narrativeSection('road-edit-section-notes', 'Anything worth noting?', '', row('road-notes-edit', 'Notes', textarea('road-notes-edit', road.notes || ''))),
    narrativeSection('road-edit-section-rating', 'How many stars?', 'Your own rating — pins it higher in your list.', starPickerHtml('road-rating-edit', rating)),
    narrativeSection('road-edit-section-stages', 'Rode it on a stage?', "Optional — link one or more stages you've ridden this road on. Dates fill in automatically.", stageLinkerHtml(linkedStageIds)),
  ];
  return narrativeShellHtml({
    id: 'rf-road-title',
    kicker: 'Edit road',
...
export function roadPayload(suffix = '') {
  return {
    road_number_or_name: fieldValue(`road-name${suffix}`),
    connection_from: fieldValue(`road-from${suffix}`) || null,
    connection_to: fieldValue(`road-to${suffix}`) || null,
    notes: fieldValue(`road-notes${suffix}`) || null,
  };
}

export function roadRatingValue(suffix = '') {
  return Number(field(`road-rating${suffix}`)?.value) || 0;
}
