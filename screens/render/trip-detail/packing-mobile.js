...
export function renderMobileItems(trip, { screen, tripHeader }) {
  ...
  const viewToggle = `<div class="rf-clean-view-toggle">${ITEM_VIEWS.map(([key, label]) => `<button class="${view === key ? 'is-active' : ''}" data-action="rf-mobile-item-view" data-value="${key}">${label}</button>`).join('')}</div>`;
  const categoryList = `<h2>Categories</h2><div class="rf-clean-card-list">${cats.map((cat, index) => { const key = slug(cat.name); const rows = itemGroupForCategory(all, key); const catDone = rows.filter(itemDone).length; const catTodo = rows.length - catDone; return `<button class="rf-clean-category ${selected === key ? 'is-active' : ''}" data-action="rf-mobile-select-category" data-category="${esc(key)}"><span>${index + 1}</span><strong>${esc(cat.name)}</strong><b>${catTodo} left · ${catDone} done</b></button>${selected === key && rows.length ? `<div class="rf-clean-category-preview">${rows.slice(0, 4).map((item) => `<span>${itemDone(item) ? '✓' : '○'} ${esc(item.name)}</span>`).join('')}</div>` : ''}`; }).join('')}</div>`;
  const itemList = `<div class="rf-clean-section-head rf-clean-item-list-head"><h2>Items</h2><div class="rf-clean-mini-pills"><button class="${filter === 'todo' ? 'is-active' : ''}" data-action="rf-mobile-item-filter" data-value="todo">To-do</button><button class="${filter === 'done' ? 'is-active' : ''}" data-action="rf-mobile-item-filter" data-value="done">Done</button></div></div><div class="rf-clean-card-list">${filteredItems.map((item) => `<article class="rf-clean-item-card is-split ${itemDone(item) ? 'is-done' : 'is-todo'}"><button class="rf-clean-item-tap" data-action="rf-mobile-toggle-item" data-item-id="${esc(item.id)}" aria-pressed="${itemDone(item) ? 'true' : 'false'}" aria-label="${esc(item.name)} — ${itemDone(item) ? 'packed. Tap to mark as to-do.' : 'to-do. Tap to mark as packed.'}"><span class="rf-clean-item-check" aria-hidden="true">${itemDone(item) ? '✓' : ''}</span><div><strong>${esc(item.name)}</strong><small>${itemCompletionLabel(item)}${item.status === 'optional' ? ' · optional' : ''}</small></div></button><div class="rf-clean-stage-foot is-pair"><button data-action="rf-edit-item" data-item-id="${esc(item.id)}">Edit</button><button class="danger" data-action="rf-delete-item" data-item-id="${esc(item.id)}">Delete</button></div></article>`).join('') || `<div class="rf-clean-empty">No ${filter === 'done' ? 'done' : 'to-do'} items yet.</div>`}</div>`;
  const body = `<main class="rf-clean-page"><section class="rf-clean-ledger rf-clean-items-ledger"><div><small>The packing list</small><strong>${done}<span> / ${all.length}</span></strong><span>${todo} left · ${done} done</span></div><button data-action="rf-mobile-add-item">+ Add item</button></section>${view === 'categories' ? categoryList : itemList}${viewToggle}</main>`;
  return screen(body, undefined, { header: tripHeader(trip, 'items') });
}
