...
export function gpxUploadWizardHtml() {
  ...
  return panelHtml({ id: 'rf-gpx-upload-title', kicker: 'GPX upload', title: 'Attach a GPX track', sub: `${stageLabel}. The selected file is captured before upload so a re-render cannot wipe it.`, errorId: 'gpx-error', saveAction: 'rf-save-gpx-upload', saveLabel: 'Upload GPX', body: [row('gpx-file', 'GPX file', `${fileInput('gpx-file', `accept=".gpx,application/gpx+xml,application/xml,text/xml" data-trip-id="${esc(tripId)}"`)}<div class="rf-desktop-aside-sub" id="gpx-selected-file">${selectedFile ? `Selected: ${esc(selectedFile)}` : 'No file selected yet.'}</div>`)].join('') }).replace('data-action="rf-cancel-wizard"', 'data-action="rf-cancel-gpx-upload"');
}
