...
function mobileWeatherHtml(stage) {
  ...
  return weatherPanelHtml(stage, wx, { prefix: 'rf-mobile' });
}
...
function stageActionFooterHtml(stage, { index, total, trip }) {
  ...
  return `<div class="rf-clean-stage-foot is-quint">`
    + `<button class="icon" data-action="rf-stage-up" data-stage-id="${esc(stage.id)}" aria-label="Move stage up" ${index === 0 ? 'disabled' : w}>↑</button>`
    + `<button class="icon" data-action="rf-stage-down" data-stage-id="${esc(stage.id)}" aria-label="Move stage down" ${index === total - 1 ? 'disabled' : w}>↓</button>`
    + `<button data-action="rf-mobile-open-stage" data-stage-id="${esc(stage.id)}">Journal</button>`
    + `<button data-action="rf-edit-stage" data-stage-id="${esc(stage.id)}"${w}>Edit</button>`
    + `<button class="danger" data-action="rf-delete-stage" data-stage-id="${esc(stage.id)}"${w}>Delete</button>`
    + `</div>`;
}
...
export function renderMobileStages(trip, { screen, tripHeader }) {
  ...
  const cards = st.map((stage, index) => `<article class="rf-clean-stage-card is-split ${revealedId === stage.id ? 'is-selected' : ''}"><button class="rf-clean-stage-tap" data-action="rf-mobile-select-stage-card" data-stage-id="${esc(stage.id)}"><span class="is-${stageNodeStatus(stage)}">${index + 1}</span><div><strong>${esc(stage.start_location || 'Start')} <em>to</em> ${esc(stage.end_location || 'End')}</strong><small>${esc(day(stage.planned_date))} · ${Math.round(Number(stage.distance_km) || 0)} km · ${esc(fmtDate(stage.planned_date) || '')}</small>${stage.notes ? `<p>${esc(stage.notes)}</p>` : ''}</div></button>${revealedId === stage.id ? stageActionFooterHtml(stage, { index, total, trip }) : ''}</article>`).join('');
  // HANDOFF.md generic empty state: centered icon-less message + a
  // "+ Add a stage" CTA, not a special-cased hero variant.
  const emptyState = `<div class="rf-desktop-empty-state"><p>No stages yet.</p><p class="rf-desktop-empty-hint">Add the first leg of this trip to start the route.</p><button class="rf-mobile-btn is-primary" data-action="rf-mobile-add-stage" type="button">+ Add a stage</button></div>`;
  const body = `<main class="rf-clean-page">${st.length ? `<div class="rf-clean-stage-rail">${cards}</div><button class="rf-mobile-btn is-dashed" data-action="rf-mobile-add-stage">+ Add another stage</button>` : emptyState}</main>`;
  return screen(body, undefined, { header: tripHeader(trip, 'stages') });
}
...
function mobileStageActionsHtml(trip, stage) {
  ...
  if (showStageActions(trip)) {
    const w = writeDisabledAttr(trip);
    html += `<button class="rf-mobile-pill-action" data-action="rf-edit-stage" data-stage-id="${esc(stage.id)}"${w}>Edit stage</button>`
          + `<button class="rf-mobile-pill-action danger" data-action="rf-delete-stage" data-stage-id="${esc(stage.id)}"${w}>Delete</button>`;
  }
  return `<div class="rf-mobile-stage-actions">${html}</div>`;
}
...
function mobileNoteHtml(entry, index, total, manual, trip) {
  const acts = showStageActions(trip)
    ? `<div class="rf-clean-note-acts">${manual ? `<button data-action="rf-journal-entry-up" data-entry-id="${esc(entry.id)}" ${index === 0 ? 'disabled' : ''} aria-label="Move up">↑</button><button data-action="rf-journal-entry-down" data-entry-id="${esc(entry.id)}" ${index === total - 1 ? 'disabled' : ''} aria-label="Move down">↓</button>` : ''}<button data-action="rf-edit-entry" data-entry-id="${esc(entry.id)}"${writeDisabledAttr(trip)} aria-label="Edit entry">✎</button><button class="danger" data-action="rf-delete-entry" data-entry-id="${esc(entry.id)}"${writeDisabledAttr(trip)} aria-label="Delete entry">✕</button></div>`
    : '';
...
export function renderMobileJournal(trip, { screen }) {
  ...
  // Route Atlas mockup ("Journal — stage entries" frame): back button,
  // stage kicker/title, and the Navigate/Edit/Delete pill row are fixed
  // in the header, matching every other trip-detail screen — only the
  // weather panel, notes, costs and GPX section scroll.
  const header = `<header class="rf-clean-trip-head"><button class="rf-clean-back" data-action="rf-mobile-back-to-stages">← ${esc(trip.title || 'Trip')}</button><div class="rf-clean-kicker">Stage · ${esc(fmtDate(stage.planned_date) || '')}</div><h1 class="rf-clean-title">${esc(stage.start_location || 'Start')} <em>to</em> ${esc(stage.end_location || 'End')}</h1>${mobileStageActionsHtml(trip, stage)}</header>`;
  const body = `<main class="rf-clean-page"><p>${esc(stage.notes || '')}</p>${mobileWeatherHtml(stage)}<div class="rf-clean-section-head"><h2>The day's notes</h2><button data-action="rf-mobile-add-journal">+ Add</button></div>${entries.length ? journalOrderBarHtml(stage) : ''}${entries.map((entry, i) => mobileNoteHtml(entry, i, entries.length, manual, trip)).join('') || '<div class="rf-clean-empty">No entries yet.</div>'}<div class="rf-clean-section-head"><h2>Stage costs</h2><button data-action="rf-add-stage-expense" data-stage-id="${esc(stage.id)}">+ Add</button></div>${stageExpenses.map((expense) => `<article class="rf-clean-expense"><div><strong>${esc(categoryLabel(expense.category))}</strong><small>${esc(payerName(expense.user_id))}</small></div><b>${fmtEuro(expense.amount || 0)}</b></article>`).join('') || '<div class="rf-clean-empty">No costs assigned to this stage.</div>'}<section class="rf-gpx-section">${gpxPanelHtml(trip, stage, tracks)}</section></main>${STATE.navSheet && sheetStage ? navigateSheetHtml(sheetStage, STATE.navSheet) : ''}`;
  return screen(body, undefined, { header });
}
