...
export function saveUiState(user = STATE.user) {
  if (!user) return;
  try {
    localStorage.setItem(userKey(user), JSON.stringify(serialise()));
  } catch {}
}
...
export function restoreUiState(user = STATE.user) {
  ...
  try {
    ...
    STATE.selectedTripId = payload.selectedTripId || payload.viewTripId || null;
...
export function validateUiSelection() {
  ...
  if (STATE.tab === 'trips') {
    const tripId = STATE.viewTripId || STATE.selectedTripId;
    if (tripId && !hasTrip(tripId)) {
